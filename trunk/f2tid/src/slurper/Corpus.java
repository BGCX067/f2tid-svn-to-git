...
/**
 * Classe permettant de collecter un ensemble de documents pour construire le corpus
 * @author Burgert - Cadoret
 *
 */
public class Corpus {
	...
	public Corpus() throws IOException {
		...
		//String themes[]={"gaming", "design", "science", "sport", "france", "culture", "health", "movie", "tech", "entertainment"} ;
		String lien;
					...
					for (; (htmlIT != null) && htmlIT.isValid(); htmlIT.next()) {
						...
						for (Enumeration<HTML.Attribute> e = (Enumeration<Attribute>) attrSet.getAttributeNames(); e.hasMoreElements();) {	
							...
							if (lien.contains(s) && !(lien.contains(extPDF) || lien.contains(extDOC) || lien.contains(extPPT)) && cptLiens<1000) {
								cptLiens++;
								if ((cptLiens%10==0) || (cptLiens==1))
									System.out.println(themes[k] + " : " + cptLiens + "/" + nbBoucles);
